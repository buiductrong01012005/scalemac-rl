...
def _profile(case_id: str) -> str:
    if case_id.startswith("joint_"):
        return "joint"
    if case_id.startswith("uegroup_sum_jis_"):
        return "uegroup_sum_jis"
    if case_id.startswith("uegroup_sum_"):
        return "uegroup_sum"
...
def build_ue_group_sum_analysis(
    *, round_dir: Path, output_dir: Path, device: str = "cpu", diagnostic_slots: int = 512
        ...
        case_rows.append(
            {
                ...
                "tail_disc_is_penalty": float(tail["disc_is_penalty"].mean()) if "disc_is_penalty" in tail else 0.0,
    ...
    for profile in ("joint", "uegroup_sum", "uegroup_sum_jis"):
        ...
        profile_rows.append(
            {
                ...
                "mean_tail_disc_is_penalty": mean(row["tail_disc_is_penalty"] for row in rows),
    ...
    decision = {
        ...
        "method_note": (
            "uegroup_sum clips priority/demand importance ratios per dimension, sums the two terms within each UE, "
            "then SUMS active UE terms inside each transition before minibatch averaging. "
            "uegroup_sum_jis adds mean per-UE 2-D J_IS control while scaling the J_IS penalty with the same UE sum."
        ),
    }

    paths = {
        "case": output_dir / "ue_group_sum_case_metrics.csv",
        "profile": output_dir / "ue_group_sum_profile_summary.csv",
        "diagnostics": output_dir / "ue_group_sum_final_diagnostics.csv",
        "decision": output_dir / "ue_group_sum_decision.json",
        "html": output_dir / "ue_group_sum_ppo.html",
    }
    ...
    paths["html"].write_text(
        "<!doctype html><html><head><meta charset='utf-8'><title>Round19B</title>"
        ...
        "<h1>Round 19B — UE-group SUM Scale Correction</h1>"
